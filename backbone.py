...
@author: albert skovgaard bisgaard, albert.bisgaard@gmail.com
"""
...
def power2time(dat, ss, fs_v, st_light, sl_light):
    ...
    if idx_ss.size == 0:
        ...
        f, P = signal.periodogram(
            x, fs=fs_v, window=np.hanning(len(x)), nfft=len(x), axis=0)
    return f, P
...
def get_hours(recording, user_hour, user_date):
    user_day0 = datetime.strptime(user_date[0].replace(')', '').replace(
        '(', ''), '%m, %d, %Y')  # changes user input from GUI to date form
    user_day1 = datetime.strptime(user_date[1].replace(
        ')', '').replace('(', ''), '%m, %d, %Y')
    ...
    if abs((user_day0 - recording[0]).days) <= 2 or abs((user_day1 - recording[1]).days) <= 2:
        if (user_day1 - user_day0).days == 0:  # If hours of interest are on same day
            ...
            n_hours = abs((user_day1 - user_day0).days) * \
                24 - (user_hour[0] - user_hour[1])

        # Compute number of epochs (assumes 4 seconds epochs)
        n_epochs = ((recording[1] - recording[0]).seconds + 4) / 4
        epochs_per_hour = n_epochs / \
            (((recording[1] - recording[0]).seconds + 4) /
             3600)  # Epochs per hour
        epochs = n_hours * epochs_per_hour  # Compute number of epochs that user wants
        # Find the begining by comparing user begin time and recording begin time
        st_light = abs(recording[0].hour - user_hour[0])*epochs_per_hour
...
def get_hours2(recording, user_hour):
    ...
    st_light, sl_light, hours_dif = find_time_idx(
        p, user_hour[0], user_hour[1])
...
def get_filtered_data(in_ID, in_edf, in_tsv, in_electrode, group_nr, fs, af, bf, cf, folder_path):
    cluster_size = 4
    epoch_size = fs*cluster_size  # cluster and epoch_size

    b, a = signal.butter(af/2, [bf/(fs/2), cf/(fs/2)],
                         'bandpass', analog=False)  # Bandpass filter
    #Initialization
    edf_files = []
    tsv_files = []
    id_input = []
    ...
    if os.path.exists(folder_path):
        ...
        m = match_files(folder_path)
        count = 0
        for key in m:
            if len(m[key]) == 2:
                count = + 1
                for k in range(len(m[key])):
                    if m[key][k].endswith('.edf'):
                        edf_files.append(add_path_separator(
                            folder_path, m[key][k]))
                    if m[key][k].endswith('.tsv'):
                        tsv_files.append(add_path_separator(
                            folder_path, m[key][k]))
                id_input.append(count)
    else:
        #Cleaning input such that only files that exist are included
        # Loops over number of possible mice len(tf[1][1]) ==> this could also be a user input, i.e. maximum number of mice for all groups
        for i in range(len(in_edf[group_nr][1])):
            if os.path.exists(in_edf[group_nr][1][i][0]) and in_edf[group_nr][1][i][0].endswith('.edf'):
                edf_files.append(in_edf[group_nr][1][i][0])
            if os.path.exists(in_tsv[group_nr][1][i][0]) and in_tsv[group_nr][1][i][0].endswith('.tsv'):
    ...
    if len(range(min(len(edf_files), len(tsv_files)))) > 0:
        for i in range(min(len(edf_files), len(tsv_files))):
            ...
            Time_start = datetime.strptime(Convert(tsv_file[Time_start_idx])[
                                           2], '%m/%d/%Y %H:%M:%S')
            Time_end = datetime.strptime(Convert(tsv_file[Time_end_idx])[
                                         2], '%m/%d/%Y %H:%M:%S')
            # id_def_time[#mice][2 (0 = start time, 1 = end time)]
            id_def_time.append([Time_start, Time_end])
            ...
            if in_electrode[group_nr][1][i][0].isnumeric():
                electrode = int(in_electrode[group_nr][1][i][0]) - 1
            else:
                electrode = 0
                print("ERROR. Electrode number not numeric for group " + str(group_nr) +
                      " and mouse " + str(i) + ". Automatically using electrode = 1.")
            ...
            #Store data
            data_temp = [id_input[i], [np.reshape(
                EEG, (int(len(EEG)/epoch_size), epoch_size)), scores]]
            data2.append(data_temp)
            # indicator for number of proper defined mice
...
def get_filtered_data2(in_ID, in_tsv, group_nr, folder_path):
    ...
    if os.path.exists(folder_path):
        ...
        #Cleaning input such that only files that exist are included
        # Loops over number of possible mice len(tf[1][1]) ==> this could also be a user input, i.e. maximum number of mice for all groups
        for i in range(len(in_tsv[group_nr][1])):
            if os.path.exists(in_tsv[group_nr][1][i][0]) and in_tsv[group_nr][1][i][0].endswith('.tsv'):
                tsv_files.append(in_tsv[group_nr][1][i][0])
                id_input.append(in_ID[group_nr][1][i][0])
    #Loop through all mice
...
def clean_subsequences(subs, min_len):
    ...
    # Find index of all subs to be cleaned
    index = np.where(lengths < min_len)[0]
    # Find index of all subs to be kept
    idx_keep = list(set(index) ^ set(range(len(subs))))

    for i in range(len(index)):
        # handle if first observation is not a real bout. Then carry backwards
        if sum(idx_keep < index[i]) == 0:
...
def count_scores_all_mice(dat, ss, min_len, id_def_time, light_start, light_end, bin_limit_hour):
    l = []
    for i in range(len(dat)):
        if bin_limit_hour:
            idx_start, idx_end = light_hours_idx(
                id_def_time[i], light_start, light_end)
        else:
            idx_start = 0
            idx_end = 21599
        #arr = dat[i][1][2]
...
def hist_dat(bin_max, arr):
    ...
    for i in range(len(bin_max)):
        ...
        if bin_max[i] > 60:
            SI_upper = " min "
        labs.append(str(convert_seconds_to_minutes(prev)) + SI_lower + "< " +
                    "B <= " + str(convert_seconds_to_minutes(bin_max[i])) + SI_upper)
        if bin_max[i] > 60:
            SI_lower = " min "
    hist_nr.append(sum(np.array(arr) > bin_max[i]))
    labs.append(
        "B > " + str(convert_seconds_to_minutes(bin_max[i])) + SI_lower)
    return labs, hist_nr
...
def get_arr_from_idx(arr, idx_start, idx_end):
    if idx_end >= idx_start:
        ...
        arr = np.roll(arr, len(arr) -
                      idx_start)[:(len(arr) - idx_start + idx_end)]
    return arr
